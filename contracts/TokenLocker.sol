...
contract TokenLocker is ReentrancyGuard, HederaTokenService {
    ...
    // a public variable to hold a flag to whether owner can associate tokens to the contract or everybody else
    bool public onlyOwnerCanAssociate = false;

    // Declares a constant to represent the maximum allowed fee for the contract.
    uint256 public constant MAX_FEE = 10_000_000_000; // 10_000_000_000 Tinybar = 100 HBAR

    // Declares a constant to represent the minimum balance required for contract for auto-renewal.
    uint256 public constant AUTO_RENEW_CONTRACT_BALANCE = 10_000_000_000; // 10_000_000_000 Tinybar = 100 HBAR. HOLD 100 HBAR in The contract for AUTO_RENEW
    ...
    // Declares a receive function that accepts incoming Hbar transfers.
    receive() external payable {}

    // Declares a fallback function that accepts incoming Hbar transfers with non-empty data.
    ...
    // account address => token address => LockedToken struct
    ...
    function associateToken(address token) internal {
        ...
        // limit the association if only the owner is supposed to do so
    ...
    event TokenAllowanceApproved(address indexed token, uint256 amount);
